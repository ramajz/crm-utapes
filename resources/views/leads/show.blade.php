                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden">
                        <div class="px-5 py-4 border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-white flex items-center gap-2">
                            ...
                            <h3 class="font-semibold text-slate-900">Template Pesan WhatsApp</h3>
                        </div>
                        <div class="p-4 space-y-3">
                            @php $waNumber = $lead->customer?->wa_number ?? ''; @endphp
                            @forelse(\App\Models\WhatsAppTemplate::active()->orderBy('category')->orderBy('name')->get() as $template)
                            <div class="border border-slate-200/70 rounded-xl p-3.5 hover:border-emerald-300/70 hover:shadow-sm transition-all duration-200"
                                x-data="{ message: @js($lead->renderTemplate($template->message)) }">
                                ...
                                <textarea x-model="message" rows="3" x-ref="input-{{ $template->id }}"
                                    class="block w-full rounded-lg border-slate-200 shadow-sm focus:border-emerald-400 focus:ring-emerald-500/20 text-xs text-slate-600 resize-none bg-slate-50/50 mb-2"
                                    placeholder="Edit pesan sebelum kirim..."></textarea>
                                <a :href="'https://wa.me/{{ $waNumber }}?text=' + encodeURIComponent(message)" target="_blank"
                                    data-wa-lead-id="{{ $lead->id }}"
                                    class="wa-chat-button w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 border border-transparent rounded-lg text-xs font-semibold text-white hover:from-emerald-600 hover:to-emerald-700 transition-all duration-200 shadow-sm shadow-emerald-200 active:scale-[0.98]">
