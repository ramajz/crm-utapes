                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden">
                        ...
                        <div class="p-4 space-y-3">
                            @php $waTemplates = config('whatsapp_templates.templates', []); @endphp
                            ...
                            <div class="border border-slate-200/70 rounded-xl p-3.5 hover:border-emerald-300/70 hover:shadow-sm transition-all duration-200">
                                <div class="flex items-center justify-between mb-1.5">
                                    ...
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide
                                        @if(($template['category'] ?? 'cold') === 'hot') bg-rose-50 text-rose-600
                                        @elseif(($template['category'] ?? 'cold') === 'warm') bg-amber-50 text-amber-600
                                        @else bg-sky-50 text-sky-600 @endif
                                    ">{{ $template['category'] ?? 'cold' }}</span>
                                </div>
                                <p class="text-xs text-slate-500 leading-relaxed line-clamp-2">{{ $rendered }}</p>
                                <a href="{{ $waUrl }}" target="_blank"
                                    data-wa-lead-id="{{ $lead->id }}"
                                    class="wa-chat-button mt-2.5 w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 border border-transparent rounded-lg text-xs font-semibold text-white hover:from-emerald-600 hover:to-emerald-700 transition-all duration-200 shadow-sm shadow-emerald-200 active:scale-[0.98]">
